...
\section{Results and discussion}
    \subsection{Second virial coefficients}
        ...
        \ifSCB
            ...
            %approximately for $T \ge 340$K below which they start to deviate significantly. The semi-classical values computed in this work as well as those computed by Hellmann also seem to oscillate around the SCB value over the entire temperature range considered. At higher temperatures, due to the low uncertainty on the SCB value, this leads to a larger disagreement with the semi-classical values while at lower temperatures, due to larger uncertainty the agreement appears better. As such, for $T \le 60$K there appears to be a significant deviation of the semi-classical results of Hellmann. As explained earlier this can be attributed to the use of a different approach than ours.
            \begin{figure}[!htbp]
                \centering
            ...
            %In Fig. \ref{fig:B2N2AllPIDiff} we show second virial coefficient values for N$_2$ computed using PI-SCB approach with $P$ = 64 and the PI-CB approaches for different $P$. Under the reasonable assumption that the PI-SCB approach with $P$ = 64 yields the most accurate value, we compare the values computed using the CB approaches by computing their difference with the SCB value. From Fig. \ref{fig:B2N2AllPIDiff}, we can see that the CB values for all $P$ seem to oscillate around the SCB value for the entire range of temperatures considered. At higher temperatures, due to the low uncertainty on the SCB value, this leads to a larger disagreement with the semi-classical values while at lower temperatures, due to larger uncertainty the agreement appears better. Therefore, it is difficult to pinpoint the temperature below which there is significant deviation between the CB values and the SCB value.
            \begin{figure}[!htbp]
                \centering
            ...
            %In Fig. \ref{fig:B2N2AllSCBDiff} we show second virial coefficient values for N$_2$ computed using PI-SCB approach. Under the reasonable assumption that the PI-SCB approach with $P$ = 64 yields the most accurate value, we compare the values computed using lower $P$ against it. Here too, like in Figs. \ref{fig:B2N2AllCLSCDiff} and \ref{fig:B2N2AllPIDiff}, we notice that the lower $P$ results seem to oscillate over the range of temperatures considered and hence, we cannot conclusively say if the results have converged or not.
            \begin{figure}[!htbp]
                \centering
    ...
    \label{sec:chap5-tables}
        ...
        \pgfplotstabletypeset[
            ...
            every head row/.append style={before row={\caption{Second virial coefficient of nitrogen computed using PIMC method and CB based approach. Here B$^P_2$ represents the second virial coefficient results using $P$ beads or images per ring. Values in parantheses are standard uncertainties in the rightmost digit(s).}\label{tab:8sB2PICBResults}\\\toprule},after row=\midrule\endfirsthead},
            every first row/.append style={before row={\multicolumn{5}{c}{\tablename\ \thetable{} -- Continued from previous page.}\\\toprule},after row=\midrule\endhead},
            every last row/.style={after row=\bottomrule},
        ...
        \pgfplotstabletypeset[
            ...
            every head row/.append style={before row={\caption{Second virial coefficient of nitrogen computed using PIMC method and SCB based approach. Here B$^P_2$ represents the second virial coefficient results using $P$ beads or images per ring. Values in parantheses are standard uncertainties in the rightmost digit(s).}\label{tab:8sB2PISCBResults}\\\toprule},after row=\midrule\endfirsthead},
            every first row/.append style={before row={\multicolumn{5}{c}{\tablename\ \thetable{} -- Continued from previous page.}\\\toprule},after row=\midrule\endhead},
            every last row/.style={after row=\bottomrule},
