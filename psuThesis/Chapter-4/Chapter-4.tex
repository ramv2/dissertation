...
According to Eq. \eqref{eq:s3P} the temperature at which we require $P > 64$ to achieve converged results for Model 3 is 100 K. Therefore we expect to see significant differences between our virial coefficient values for Model 3 and the corresponding reference values for $T \le 100 K$. Although for a given temperature $T$, Eq. \eqref{eq:s3P} provides a good estimate of $P$ required for achieving converged results, in practice, one might be able to achieve convergence with fewer $P$ as well.
    ...
    \label{sec:chap4-tables}
		...
		\pgfplotstabletypeset[
            ...
            every head row/.append style={before row={\caption{Second virial coefficient of hydrogen as a function of temperature for $P$ = 8 case and Model 1. Columns 2 and 3 represent the exchange and Boltzmann contributions respectively, to the second virial coefficient. Column 4 represents the final value computed by combining the values in columns 2 and 3 according to the ratio from Table . Values in parantheses are standard uncertainties in the rightmost digit(s).}\\\toprule}, after row=\endfirsthead, before row={\caption[]{Second virial coefficient of hydrogen as a function of temperature for $P$ = 8 case and Model 1. Columns 2 and 3 represent the exchange and Boltzmann contributions respectively, to the second virial coefficient. Column 4 represents the final value computed by combining the values in columns 2 and 3 according to the ratio from Table. Values in parantheses are standard uncertainties in the rightmost digit(s).}\\\toprule},after row=\midrule\endhead},
            every last row/.style={after row=\bottomrule},
